        ...
        card.innerHTML = `
            <div class="anime-card-inner">
                <div class="anime-poster">
                    <img src="${item.image || 'https://via.placeholder.com/280x400/1a1a1a/ffffff?text=No+Image'}" loading="lazy" alt="${item.title}"
                ...
                <div class="anime-info">
                    ...
                    <div class="anime-meta">
                        ...
                        ${item.genres ? `<span class="anime-genre">${item.genres[0] || ''}</span>` : ''}
                    </div>
                </div>
            </div>
        `;
...
async function openAnimeDetail(url, animeId) {
    ...
    try {
        ...
        dom.currentTitle.textContent = data.detail.title;
        ...
        showAlert('Failed to load anime details', 'error');
    }
}
    ...
    dom.content.innerHTML = `
        <div class="anime-detail">
            <div class="anime-header">
                <img src="${data.detail.img}" alt="${data.detail.title}" class="anime-poster-img" 
                ...
                <div class="anime-header-info">
                    ...
                    <p class="anime-alt">${data.detail.alternativeTitle || ''}</p>
                    
                    <div class="anime-meta-grid">
                        ${data.detail.rating ? `
                        <div class="meta-item">
                            <strong>Rating:</strong>
                            <span>${data.detail.rating}</span>
                        </div>
                        ` : ''}
                        ${data.detail.tipe ? `
                        <div class="meta-item">
                            <strong>Type:</strong>
                            <span>${data.detail.tipe}</span>
                        </div>
                        ` : ''}
                        ${data.detail.status ? `
                        <div class="meta-item">
                        ...
                        ` : ''}
                        ${data.detail.total_episode ? `
                        <div class="meta-item">
                            <strong>Episodes:</strong>
                            <span>${data.detail.total_episode}</span>
                        </div>
                        ` : ''}
                        ${data.detail.tayang ? `
                        <div class="meta-item">
                            <strong>Aired:</strong>
                            <span>${data.detail.tayang}</span>
                        </div>
                        ` : ''}
                        ${data.detail.studio ? `
                        <div class="meta-item">
                            <strong>Studio:</strong>
                            <span>${data.detail.studio}</span>
                        </div>
                        ` : ''}
                        ${data.detail.durasi ? `
                        <div class="meta-item">
                            <strong>Duration:</strong>
                            <span>${data.detail.durasi}</span>
                        </div>
                        ` : ''}
                    </div>
            ...
            ${data.detail.sinopsis ? `
            <div class="anime-synopsis">
                <h3>Synopsis</h3>
                <p>${data.detail.sinopsis}</p>
            </div>
            ` : ''}
            ...
            <div class="anime-related">
                ...
                <div class="related-grid">
                    ${data.related.map(rel => `
                        <div class="related-item" onclick="openAnimeDetail('${rel.url}', '${btoa(encodeURIComponent(rel.url)).replace(/=/g, '')}')">
                            ${rel.title}
                        </div>
                    `).join('')}
                </div>
...
function renderEpisodes(episodes, animeUrl) {
    ...
    episodes.forEach((episode, index) => {
        ...
        btn.textContent = `Ep. ${episode.index || index + 1}: ${episode.title.substring(0, 30)}${episode.title.length > 30 ? '...' : ''}`;
        ...
        btn.onclick = () => {
            ...
            openEpisode(episode.link, episode.title);
        };
